...
var getUrl = function (opt, name, img) {
  ...
  if (opt.spriteUrlBase.indexOf('//') > -1) {
    return url.resolve(opt.spriteUrlBase, name + '.' + img.type) + cachebuster;
  }
  else {
    return path.join(opt.spriteUrlBase, name + '.' + img.type).replace(/\\/g, '/') + cachebuster;
  }
};
