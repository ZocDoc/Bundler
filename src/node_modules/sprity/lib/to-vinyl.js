      ...
      if (obj) {
        if (obj.style) {
          ...
          stream.push(new File({
            base: path.join(opt.cssOutputPath, filePath.dirname),
            relative: path.join(filePath.dirname, filePath.name + filePath.extname),
            path: path.join(opt.cssOutputPath, filePath.dirname, filePath.name + filePath.extname.toLowerCase()),
            contents: obj.style
          }));
        }
        else {
          _.each(obj.sprites, function (sprite) {
            stream.push(new File({
              base: opt.spriteOutputPath,
              relative: sprite.name + '.' + sprite.type,
              path: path.join(opt.spriteOutputPath, sprite.name + '.' + sprite.type.toLowerCase()),
